...
mod JediSwapV2NFTPositionManager {
    ...
    struct Storage {
        ...
        next_id: u256,
        next_pool_id: u64,
        fee_on_transfer_tokens: LegacyMap<ContractAddress, bool>,
        #[substorage(v0)]
    ...
    impl JediSwapV2NFTPositionManagerImpl of super::IJediSwapV2NFTPositionManager<ContractState> {
        ...
        fn create_and_initialize_pool(
            ref self: ContractState,
            token0: ContractAddress,  // This will be our fee token
            token1: ContractAddress,
            fee: u32,
            sqrt_price_X96: u256
        ) -> ContractAddress {
            // Register token0 as a fee-on-transfer token before any pool operations
            self.fee_on_transfer_tokens.write(token0, true);
        ...
        fn jediswap_v2_mint_callback(
            ...
            // Check which token has transfer fees
            let token0_has_fee = self.has_transfer_fee(decoded_data.pool_key.token0);
            let token1_has_fee = self.has_transfer_fee(decoded_data.pool_key.token1);
        
            if (amount0_owed > 0) {
                // For fee token, transfer desired amount, otherwise transfer exact owed amount
                let amount0_to_transfer = if token0_has_fee {
                    decoded_data.amount0_desired
                } else {
                    amount0_owed
                };
        
                pay(decoded_data.pool_key.token0, decoded_data.payer, caller, amount0_to_transfer);
            }
        
            if (amount1_owed > 0) {
                // For fee token, transfer desired amount, otherwise transfer exact owed amount
                let amount1_to_transfer = if token1_has_fee {
                    decoded_data.amount1_desired
                } else {
                    amount1_owed
                };
        
                pay(decoded_data.pool_key.token1, decoded_data.payer, caller, amount1_to_transfer);
            }
        }
    ...
    impl InternalImpl of InternalTrait {
        // @dev Helper function to check if a token has transfer fees
        fn has_transfer_fee(
            self: @ContractState,
            token: ContractAddress,
        ) -> bool {
            self.fee_on_transfer_tokens.read(token)
        }
        ...
        fn _add_liquidity(
            ...
            // Check which token has transfer fees
            let token0_has_fee = self.has_transfer_fee(params.token0);
            let token1_has_fee = self.has_transfer_fee(params.token1);
        
            // Use min amounts for fee tokens and desired amounts for standard tokens
            let amount0_for_liquidity = if token0_has_fee {
                params.amount0_min
            } else {
                params.amount0_desired
            };
        
            let amount1_for_liquidity = if token1_has_fee {
                params.amount1_min
            } else {
            ...
            let liquidity = get_liquidity_for_amounts(
                ...
                sqrt_ratio_a_X96,
                sqrt_ratio_b_X96,
                amount0_for_liquidity,
                amount1_for_liquidity
            );
        
            // Modify MintCallbackData to include desired amounts
            ...
            // For fee tokens, check against min_amount
            // For standard tokens, check against calculated amount
            let amount0_check = if token0_has_fee { params.amount0_min } else { amount0 };
            let amount1_check = if token1_has_fee { params.amount1_min } else { amount1 };
        
            assert(
                amount0 >= amount0_check && amount1 >= amount1_check,
